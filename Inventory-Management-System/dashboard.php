...
require_once 'api/ForecastingUtil.php';

$inventory = [];
$productNames = [];
$productSales = [];

// Fetch all inventory items from SQL
...
if ($result && $result->num_rows > 0) {
    while ($item = $result->fetch_assoc()) {
        // Decode sales data from JSON strings
...
<body class="bg-light">
            ...
            <div class="container-fluid">
                <div class="row align-items-center mb-4">
                    ...
                    <div class="col-auto d-flex gap-2">
                        <div class="dropdown">
                            <button class="btn btn-outline-primary dropdown-toggle px-4 py-2 fw-bold shadow-sm" type="button" data-bs-toggle="dropdown">
                                <i class="bi bi-download me-2"></i> Export
                            </button>
                            <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg mt-2">
                                <li><a class="dropdown-item py-2" href="api/export_csv.php"><i class="bi bi-filetype-csv me-2 text-success"></i> Export CSV</a></li>
                                <li><a class="dropdown-item py-2" href="api/export_pdf.php"><i class="bi bi-filetype-pdf me-2 text-danger"></i> Export PDF</a></li>
                            </ul>
                        </div>
    ...
    <script>
        ...
        function renderInventoryTable(data) {
            ...
            data.forEach(item => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-4 py-3 text-center">
                        <span class="badge ai-badge px-3 py-2" title="Confidence: ${item.confidence}%">
                             <i class="bi bi-cpu me-1"></i> ${item.forecast}
                        </span>
                        <div class="mt-1" style="font-size: 0.75rem;">
                            <span class="text-${item.trend === 'up' ? 'success' : (item.trend === 'down' ? 'danger' : 'secondary')} fw-bold">
                                <i class="bi bi-graph-${item.trend === 'up' ? 'up' : (item.trend === 'down' ? 'down' : 'flat')} me-1"></i>
                                ${item.trend === 'up' ? '+' : ''}${item.trend_percentage}%
                            </span>
                        </div>
                    </td>
        ...
        document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
            if (itemToDelete) {
                fetch(`api/delete.php?id=${itemToDelete}&csrf_token=${CSRF_TOKEN}`)
                    .then(response => response.json())
                    .then(data => {
